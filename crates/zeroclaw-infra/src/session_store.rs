        ...
        match std::fs::read(self.trim_breadcrumb_path(session_key)) {
            Ok(bytes) => match bytes.as_slice() {
                ...
                _ => Err(std::io::Error::new(
                    ...
                    "Invalid session trim breadcrumb record",
                )),
            },
...
mod tests {
    ...
    #[test]
    fn trim_breadcrumb_rejects_malformed_sidecars() {
        let tmp = TempDir::new().unwrap();
        let store = SessionStore::new(tmp.path()).unwrap();
        store
            .append("chan_user", &ChatMessage::user("retained"))
            .unwrap();
        ...
        for contents in [
            ...
            std::fs::write(store.trim_breadcrumb_path("chan_user"), contents).unwrap();
            assert_eq!(
                store.get_trim_breadcrumb("chan_user").unwrap_err().kind(),
                std::io::ErrorKind::InvalidData,
            );
            assert_eq!(store.try_load("chan_user").unwrap()[0].content, "retained");
        }
    }
