...
/// exceeds the configured budget, the gate first attempts a hook/schema-aware
/// whole-turn trim of the durable history (owner-aware breadcrumb handling)
/// before declaring an unsatisfiable floor. The dispatch itself still
/// proceeds; the provider's own overflow signal and recovery path stay
...
fn surface_oversized_dispatch_if_needed(
    ...
    // This is a best-effort raw estimate (multimodal expansion and hook
    // growth are not re-estimated here); the durable trim at least ensures
    // the flag is not raised as a false floor when a whole turn could be
    // removed. The estimate after each drop is the raw history estimate plus
    // the constant tool-schema population, which matches the
    // `reported_population_estimated` shape used at this seam.
    ...
    loop {
        let dropped =
            crate::agent::history_trim::drop_oldest_whole_turn(&mut trimmed_history, had_crumb);
        if dropped == 0 {
            hit_floor = true;
            break;
        }
    ...
    if trimmed_any {
        ...
        *crumb_present = new_crumb;
        ...
        // does not overstate.
        let dropped_messages = taken_len.saturating_sub(
            with_crumb
                .len()
                .saturating_add(usize::from(had_crumb))
                .saturating_sub(1),
        );
...
impl<'a> TurnState<'a> {
    ...
    fn trim_to_budget(
        ...
        let mut result = crate::agent::history_trim::trim_to_recent_turns_with_crumb(
            taken,
            context_token_budget,
            self.crumb_present,
        );
...
/// Whether a `before_llm_call` hook's mutation of `pre_hook` into `post_hook`
/// was a pure append: every message the hook call started with is still
/// present, unmodified, at the same leading positions, and `post_hook` is at
/// least as long as `pre_hook`. Anything else — an insertion before the end,
/// a deletion, an in-place replacement, or a reorder — returns `false`.
///
/// This is the narrower contract the pre-dispatch trim rebuild in
/// [`run_tool_call_loop`] requires (see the `hook_only_appended` call site):
/// dropping whole durable turns from the post-hook prefix by position/count
/// and re-appending the measured suffix is only an identity-preserving
/// mapping back to durable turns when the hook stayed inside it. The general
/// `before_llm_call` contract in `zeroclaw-api` still allows arbitrary
/// mutation; this check exists to detect when a dispatch went outside what
/// this specific rebuild strategy can safely handle.
fn hook_mutation_is_pure_append(pre_hook: &[ChatMessage], post_hook: &[ChatMessage]) -> bool {
    post_hook.len() >= pre_hook.len()
        && post_hook
            .iter()
            .zip(pre_hook.iter())
            .all(|(post, pre)| post.role == pre.role && post.content == pre.content)
}
...
pub async fn run_tool_call_loop(mut p: ToolLoop<'_>) -> Result<String> {
    ...
    for iteration in 0..max_iterations {
        ...
        //
        // This "suffix by length" reconstruction is only an identity-
        // preserving mapping back to durable turns when the hook's mutation
        // is a pure append: the public hook contract otherwise allows a
        // `before_llm_call` handler to insert, delete, replace, or reorder
        // messages anywhere in the vector. `hook_only_appended` records
        // whether this dispatch's hook call actually stayed inside that
        // narrower contract, so the rebuild below (used only when a trim is
        // needed) can refuse to run rather than silently drop or duplicate
        // messages when a hook did something else.
        let hook_only_appended =
            hook_mutation_is_pure_append(&pre_hook_messages, &provider_request_messages);
...
#[cfg(test)]
mod hook_mutation_contract_tests {
    use super::*;

    // `hook_mutation_is_pure_append` is the gate that decides whether the
    // pre-dispatch trim rebuild in `run_tool_call_loop` may run at all: the
    // rebuild drops whole durable turns from the post-hook prefix by
    // position/count, which only maps back to durable turns correctly when
    // the hook's own mutation was a pure append. These cases cover the
    // insertion, deletion/replacement, and reordering shapes the general
    // `before_llm_call` contract permits but this narrower rebuild path does
    // not, plus the pure-append and no-op shapes it does accept.
    ...
    #[test]
    fn pure_append_is_accepted() {
        let pre_hook = sample();
        let mut post_hook = pre_hook.clone();
        post_hook.push(ChatMessage::assistant("appended by hook"));
        assert!(hook_mutation_is_pure_append(&pre_hook, &post_hook));
    }

    #[test]
    fn identical_vector_is_accepted() {
        let pre_hook = sample();
        let post_hook = pre_hook.clone();
        assert!(hook_mutation_is_pure_append(&pre_hook, &post_hook));
    }

    #[test]
    fn insertion_before_the_end_is_rejected() {
        let pre_hook = sample();
        let mut post_hook = pre_hook.clone();
        post_hook.insert(1, ChatMessage::system("hook-inserted mid-vector"));
        assert!(!hook_mutation_is_pure_append(&pre_hook, &post_hook));
    }
    ...
    #[test]
    fn in_place_replacement_is_rejected() {
        let pre_hook = sample();
        let mut post_hook = pre_hook.clone();
        post_hook[0] = ChatMessage::user("rewritten by hook");
        assert!(!hook_mutation_is_pure_append(&pre_hook, &post_hook));
    }
