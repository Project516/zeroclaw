...
channel-runtime-history-unavailable = ⚠️ Your conversation history could not be verified right now. Please try again in a moment.
...
turn-hook-mutation-unsupported-for-trim = A before_llm_call hook modified this turn's request beyond appending messages (insert, delete, replace, or reorder), and the conversation history needs to be trimmed to fit the configured context budget. Trimming cannot safely rebuild a request whose hook mutation is not a pure append. Use a hook that only appends messages, or raise the configured context budget.
